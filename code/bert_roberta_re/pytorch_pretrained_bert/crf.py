...
class CRF(nn.Module):
    ...
    def __init__(self, tagset_size, gpu, use_predefined_matrix=False, transition_matrix=None):
        ...
        if use_predefined_matrix:
            init_transitions = torch.zeros(self.tagset_size + 3, self.tagset_size + 3)
            for i in range(len(transition_matrix)):
                for j in range(len(transition_matrix[i])):
                    init_transitions[i][j] = transition_matrix[i][j]
            # init_transitions = torch.tensor(transition_matrix)
            assert init_transitions.shape == (self.tagset_size+3, self.tagset_size+3)
        else:
            init_transitions = torch.zeros(self.tagset_size+3, self.tagset_size+3)
        ...
        # self.transitions = nn.Parameter(torch.Tensor(self.tagset_size+3, self.tagset_size+3))
    ...
    def _calculate_PZ(self, feats, mask):
        ...
        assert(tag_size == self.tagset_size+3)
    ...
    def _viterbi_decode(self, feats, mask):
        ...
        pad_zero = autograd.Variable(torch.zeros(batch_size, tag_size)).long().cuda()
        if self.gpu:
            pad_zero = pad_zero.cuda()
        back_points.append(pad_zero)
        #
        # back_points = torch.tensor(back_points)
        # back_points = back_points.cuda()
    ...
    def _score_sentence(self, scores, mask, tags):
        ...
        # print('tg_energy',tg_energy.size())
#         print('end_energy',end_energy.size())
#         print(tg_energy + end_energy)
#         gold_score_1 =  tg_energy + end_energy
#         print(gold_score_1.size())
        print('tg_energy',tg_energy.size())
        print('tg_energy.sum',tg_energy.sum())
        gold_score = tg_energy.sum() + end_energy.sum()
        # print(gold_score.size())
        return gold_score

    def neg_log_likelihood_loss(self, feats, mask, tags):#求一对《sentence,tag》在当前参数下的负对数似然
        # nonegative log likelihood
        batch_size = feats.size(0)#
        forward_score, scores = self._calculate_PZ(feats, mask)#所有的路径的分数和
        # print(scores)
        
        gold_score = self._score_sentence(scores, mask, tags) #正确的路径的分数
