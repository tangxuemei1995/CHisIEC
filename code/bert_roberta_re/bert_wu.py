...
class Classification(nn.Module):
    def __init__(self,config):
        ...
        self.full_con_Layer_2 = nn.Linear(hidden_size*2,self.num_labels,bias=True)
        ...
        if config.use_cnn:
            #使用PCNN，初始化CNN
            self.conv1 = nn.Conv1d(in_channels=hidden_size, out_channels=20, kernel_size = 2,)
...
#                 # print(average_e1.size())
                e1_embedding = self.tanh(average_e1).to(config.device)
                e2_embedding = self.tanh(average_e2).to(config.device)
#
...
#
                ...
                h = torch.cat([e1, e2],-1).to(config.device)
                h = torch.mm(h, self.full_con_Layer_2.weight.t().to(config.device)) + self.full_con_Layer_2.bias.to(config.device)
                
        
        logits = self.softmax(h)
        l_logits = self.logsoftmax(h)
...
def load_and_cache_examples(config, task, tokenizer, evaluate=False, test=False):
    ...
    dataset = TensorDataset(all_input_ids, all_input_mask,
                            all_segment_ids, all_label_ids, all_e1_mask, all_e2_mask, all_e1_e2_mask )
    return dataset
...
def main():
    ...
    # Prepare GLUE task
    processor = data_processors["cclue"]()
    output_mode = output_modes["cclue"]
    label_list = processor.get_labels()
    # print(label_list)
...
#                                 exit()
                                ...
                                precision = sum(precision)/48 #实际只有48个标签
                                recall = sum(recall)/48 #实际只有48个标签
                                ...
                                f = open(config.output_dir + "cclue_m.txt",'a',encoding='utf-8')
                                ...
                                output_eval_file = config.output_dir + "cclue_true_pred.txt"
    ...
    if config.test:
        ...
        test_precision = sum(test_precision)/48 #实际只有48个标签
        test_recall = sum(test_recall)/48 #实际只有48个标签
        test_f1_micor = (2*test_precision*test_recall)/(test_precision+test_recall)
                                
                                
        test_result = {
                   "test_acc": test_acc,
                   "test_宏平均f1": test_f1,
                   'tets_p': test_precision,
                   'tets_r': test_recall,
                   "test微平均f1": test_f1_micor ,
               }
        ...
        f = open(config.output_dir + "cclue_test_m.txt",'a',encoding='utf-8')
        # f.write('epoch:' + str(epoch)+'\n' )
        for key in sorted(test_result.keys()):
               print("  %s = %s", key, str(test_result[key]))
               f.write(key+'\t' +str(test_result[key])+'\n')
        f.write('\n')
        output_eval_file = config.output_dir + "cclue_test_true_pred.txt"
        with open(output_eval_file, "w") as writer:
               for key in range(len(y_true)):
                   writer.write("%d\t%s\t%s\n" %
                                (key, str(RELATION_LABELS[y_true[key]]),str(RELATION_LABELS[y_pre[key]])))
